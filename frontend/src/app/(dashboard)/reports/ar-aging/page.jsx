...
function AgingPage({ type }) {
  ...
  const fetch = () => {
    setLoad(true);
    const call = type === 'ar' ? reportsApi.arAging(asOfDate) : reportsApi.apAging(asOfDate);
    call.then(r => setData(r.data))
  ...
  const entity = type === 'ar' ? 'customer' : 'supplier';

  return (
    <div>
      <div className="page-header">
        <div>
          <h1 className="page-title">{label} Report</h1>
          <p className="page-subtitle">Outstanding {type === 'ar' ? 'invoices' : 'bills'} by age</p>
        </div>
      ...
      {data && !loading && (
        <>
            ...
            {BUCKET_ORDER.map(bucket => {
              ...
              return (
                <div key={bucket} className="card-hover p-4 text-center">
                  ...
                  <p className="text-xs text-muted">{b?.invoices?.length || b?.bills?.length || 0} records</p>
                </div>
              );
            })}
          </div>
          ...
          {BUCKET_ORDER.filter(b => data.buckets[b]).map(bucket => {
            const items = data.buckets[bucket]?.invoices || data.buckets[bucket]?.bills || [];
              ...
              <div key={bucket} className="card overflow-hidden mb-4">
                ...
                <div className="table-container">
                  <table className="table">
                    <thead><tr>
                      <th>#</th><th className="capitalize">{entity}</th>
                      <th>Due Date</th><th>Days Overdue</th>
                      <th className="text-right">Amount Due</th>
                    </tr></thead>
                    <tbody>
                      {items.map(item => (
                        <tr key={item.invoice_id || item.bill_id}>
                          <td className="font-mono text-xs text-primary-light">{item.invoice_number || item.bill_number}</td>
                          <td>{item.customer_name || item.supplier_name}</td>
                          <td className="text-muted text-xs">{item.due_date}</td>
                          <td>
                            <span className={`badge ${BUCKET_STYLES[bucket]}`}>
                              {item.days_overdue <= 0 ? 'Current' : `${item.days_overdue}d`}
                            </span>
                          </td>
...
export function ARAgingPage() { return <AgingPage type="ar"/>; }
export function APAgingPage() { return <AgingPage type="ap"/>; }
